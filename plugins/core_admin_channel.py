...
def is_globaladmin(hostmask, chan, bot):
    ...
    if globaladmins:
        return compare_hostmasks(hostmask, globaladmins)


def is_channeladmin(hostmask, chan, db):
    channeladmins = database.get(
        db,
        'channels',
        'admins',
        'chan',
...
def admins(inp, notice=None, bot=None, chan=None, db=None):
    ...
    if admins:
        notice(u"[{}]: Admins are: {}".format(chan, admins))
    else:
        notice(u"[{}]: No nicks/hosts are currently admins.".format(chan))
    return
        ...
        if '*' in nicks:
            ...
            for nick in nicks:
                ...
                if admins and nick in admins:
                    ...
                    database.set(
                        db, 'channels', 'admins', admins, 'chan', chan)
                    notice(
                        u"[{}]: {} is no longer an admin.".format(
                            chan, nick))
                else:
...
# @hook.command(permissions=["op_lock", "op"], channeladminonly=True,
#               autohelp=False)
...
#     else:
#         s = u"[{}]: No nicks/hosts are currently auto opped."
#         notice(s.format(chan))
#     return
...
def autoop(inp, notice=None, bot=None, chan=None, db=None):
    '''
    aop [channel] <enable|disable> OR <add|del> <nick|host>
    Add/Del Autoops
    '''
    autoops = database.get(db, 'channels', 'autoops', 'chan', chan)
...
def disabled(inp, notice=None, bot=None, chan=None, db=None):
    ...
    if disabledcommands:
        notice(u"[{}]: Disabled commands: {} {}".format(
...
def disable(inp, notice=None, bot=None, chan=None, db=None):
    ...
    for target in targets:
        if disabledcommands and target in disabledcommands:
            ...
            if 'disable' in target or 'enable' in target or 'core_admin' \
                    in target:
                ...
                for commands in disabledcommands:
                    if target == commands:
                        ...
                        database.set(
                            ...
                            chan)
                        notice(
                            u"[{}]: {} is now enabled.".format(
                                chan, target))
                    else:
                        pass
            else:
                if target in " ".join(bot.config["disabled_commands"]):
                    s = u"[{}]: {} is globally disabled. Use .genable {} " \
                        "to enable."
                    notice(s.format(chan, target, target))
                else:
...
def showfloods(inp, chan=None, notice=None, db=None):
    ...
    if flood:
        notice(u"[{}]: Flood: {} messages in {} seconds".format(
            chan, flood.split()[0], flood.split()[1]))
    else:
    ...
    if cmdflood:
        notice(u"[{}]: Command Flood: {} commands in {} seconds".format(
            chan, cmdflood.split()[0], cmdflood.split()[1]))
    else:
...
def flood(inp, conn=None, chan=None, notice=None, db=None):
    '''
    flood [channel] <number> <duration> | disable
    Enables flood protection for a channel.
    ...
    if len(inp) == 0:
        ...
        s = u"[{}]: Flood Protection limited to {} messages in {} seconds."
...
def trim(inp, conn=None, chan=None, notice=None, db=None):
    ...
    if len(inp) == 0:
        ...
        if trimlength:
            notice(
                u"[{}]: Trim output set to {} characters.".format(
                    chan, trimlength))
        else:
...
def bans(inp, notice=None, bot=None, chan=None, db=None):
    ...
    if bans:
        notice(u"[{}]: Bans are: {}".format(chan, bans))
    else:
        notice(u"[{}]: No nicks/hosts are in the banlist.".format(chan))
    return
...
def invite(inp, conn=None, chan=None, notice=None, ):
    ...
    for a_user in users:
        # slows down legit invites but better than getting pastabot kicked
        # still possible to get pastabot kicked but much harder
        if throttle <= 10:
            sleeptime = len(users) / 3
            # just an extra fuck you to people who do huge invites
            if len(users) >= 20:
                sleeptime += len(users)
            users, throttle = nestedinvite(a_user, throttle, sleeptime, users)
        else:
            sleep(10)
            throttle = 0
...
def remove(inp, chan=None, conn=None):
    ...
    Force a user to part from a channel.
    '''
    message = " ".join(inp)
