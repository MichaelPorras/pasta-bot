...
def version(inp, nick=None, chan=None, conn=None, notice=None):
    ...
    if not user:
        user = nick
    if 'pasta-bot' in user:
        m = '[VERSION] pasta-bot: SkyNet 0.99 kawaii disrespecting humanitys '\
            'freedom edition'
        return m
    # ctcpcache_timer
        ...
        if ctcpcache:
            for x in ctcpcache:
                ...
                if re.search(
                        ...
                        re.escape(nick),
                        inpnick,
                        re.I):  # .replace('[','\[').replace(']','\]')
                    ctcpcache.remove(x)
                    if kind == "VERSION":
                        conn.send(
                            u"PRIVMSG {} :[{}] {}: {}".format(
                                channel, kind, nick, inpresult))
                        return
                    elif kind == "PING":
                        ...
                        if senttime:
                            ...
                            if diff <= 1:
                                conn.send(
                                    u"PRIVMSG {} :[{}] {}: {} ms".format(
                                        channel, kind, nick, diff * 1000))
                            else:
                                conn.send(
                                    u"PRIVMSG {} :[{}] {}: {} seconds".format(
                                        channel, kind, nick, diff))
                            return
                        else:
                            return
